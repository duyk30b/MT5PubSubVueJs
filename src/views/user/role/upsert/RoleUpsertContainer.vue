...
<template>
  ...
  <form class="md:mx-4 mt-4 p-4 bg-white" @submit.prevent="handleSave">
      ...
      <template #panel>
        ...
        <VueTabPanel :tabKey="TABS_KEY.ROLE_PERMISSION">
          <div class="mt-4">
            ...
            <div class="mt-4 table-wrapper" style="max-height: 600px; border-bottom: 1px solid #cdcdcd">
              <table>
                <thead>
                  <tr>
                    <th style="width: 30px">
                      <div class="flex justify-center">
                        <input style="cursor: pointer" type="checkbox" :checked="checkedOptions.checkedAll"
                          :indeterminate="checkedOptions.indeterminate" @change="(e) => handleChangeCheckAll(e)" />
                      </div>
                    </th>
                  ...
                  <template v-for="level1 in permissionAllTree" :key="level1.id">
                    <tr style="user-select: none">
                      <td class="text-center">
                        <div class="flex justify-center">
                          <input :id="randomId + '_' + level1.id" style="cursor: pointer"
                            :indeterminate="checkedOptions.level1Map[level1.id].indeterminate"
                            :checked="checkedOptions.level1Map[level1.id].checkedAll" type="checkbox"
                            @change="(e) => handleChangeCheckLevel1(e, level1.id)" />
                        </div>
                      </td>
                      <td colspan="2" class="font-medium">
                        <label style="cursor: pointer; user-select: none" :for="randomId + '_' + level1.id">
                    ...
                    <template v-for="child in level1.children" :key="child.id">
                      <tr style="user-select: none">
                        ...
                        <td class="text-center" style="width: 30px">
                          <div class="flex justify-center">
                            <input :id="randomId + '_' + child.id" style="cursor: pointer" :checked="checkedOptions.level1Map[level1.id].level2Map[child.id] ||
                              checkedOptions.level1Map[level1.id].checkedAll
                              " type="checkbox" @change="(e) => handleChangeCheckLevel2(e, level1.id, child.id)" />
                          </div>
                        </td>
                        <td>
                          <label style="cursor: pointer; user-select: none" :for="randomId + '_' + child.id">
    ...
    <div class="mt-8 flex gap-4">
      ...
      <VueButton color="blue" type="submit" :loading="saveLoading" icon="save" style="margin-left: auto">
        Lưu lại
      </VueButton>
    </div>
  </form>
