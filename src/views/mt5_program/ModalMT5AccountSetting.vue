...
const copyMasterLoginOptions = computed(() => {
  return mt5ProgramStore.mt5ProgramDataList
    .filter(
      (program) =>
        program.account_setting?.mt5_account?.accountLogin &&
        program.account_setting.mt5_account.accountLogin !== mt5Account.value.accountLogin &&
        program.account_setting?.mt5_account?.accountType === Mt5AccountType.MASTER,
    )
    .map((program) => ({
      label: `${program.information.account_info.login} - ${program.information.account_info.name}`,
      value: program.information.account_info.login,
    }))
})
...
const openModal = async (props: { accountLogin: number; programName: string }) => {
  ...
  if (!mt5AccountOrigin.value.id) {
    const mt5Program = mt5ProgramStore.mt5ProgramDataList.find(
      (program) => program.information?.account_info?.login === accountLogin,
    )
    if (mt5Program) {
      mt5AccountOrigin.value.accountLogin = mt5Program.information.account_info.login
      mt5AccountOrigin.value.accountName = mt5Program.information.account_info.name
      mt5AccountOrigin.value.accountServer = mt5Program.information.account_info.server
    }
  }
